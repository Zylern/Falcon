...
from userbot.falcon_userbot.heroku_var import *
from userbot import *
from sys import *
...
def blackeye(pattern=None, **args):
    args["func"] = lambda e: e.via_bot_id is None
    stack = inspect.stack()
    previous_stack_frame = stack[1]
    file_test = Path(previous_stack_frame.filename)
        ...
        async def wrapper(check):
            ...
            try:
                ...
                if not disable_errors:
                    ...
                    ftext += result
                    file = open("falcon_error.log", "w+")
                    file.write(ftext)
                    file.close()
                    try:                 
                      await check.client.send_file(send_to, "falcon_error.log", caption=text)
                      remove("falcon_error.log")
                    except:
                      pass
...
borg = falcon = bot ; admin_cmd = blackeye ; command = rewtw ; register = zylern = zylernn
...
data = json.load(open("userbot/falcon_userbot/extra/meaning.json")) 
def meaning(w): 
	w = w.lower() 
	if w in data: 
		return data[w] 
